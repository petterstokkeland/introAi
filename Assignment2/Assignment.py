...
class CSP:
    def __init__(self):
        ...
        self.constraints = {}
        self.backTrackSucess = 0
        self.backTrackFail = 0
    ...
    def backtrack(self, assignment):
        ...
        """
        self.backTrackSucess += 1
        if all(len(assignment[var]) == 1 for var in self.variables):
            return assignment
       
        var = self.select_unassigned_variable(assignment)
       
        #print("Back track sucess: " + self.backTrackSucess + "Back track fail: " + self.backTrackFail)
        for value in assignment[var]:
            ...
            if(self.inference(new_assignment, self.get_all_arcs())):
                ...
                if(result != None):
                    return result
        self.backTrackFail += 1

        return None

    def print_backtrack_stats(self):
        """This function prints the number of times the backtrack function
        was called with a partial assignment that was successful, and
        the number of times it was called with a partial assignment
        that failed.
        """
        print("Backtrack success: %d, Backtrack fail: %d" %
              (self.backTrackSucess, self.backTrackFail))
        ...
        while len(queue) > 0:
            ...
            if self.revise(assignment, x, y):
                if len(assignment[x]) == 0:
                    return False
                for node in self.get_all_neighboring_arcs(x):
    ...
    def revise(self, assignment, i, j):
        ...
        # Sjekk om det finnes en verdi y i j's domene slik at (x, y) oppfyller restriksjonen
